...
def draw_load_menu(index):
    ...
    loading_btn = pygame.draw.rect(
        screen, gray, [WIDTH // 2 - 200, HEIGHT * 0.87, 400, 90], 0, 5)
    ...
    delete_btn = pygame.draw.rect(
        screen, gray, [(WIDTH // 2) - 500, HEIGHT * 0.87, 200, 90], 0, 5)
    delete_text = label_font.render("Delete Beat", True, white)
    screen.blit(delete_text, ((WIDTH // 2) - 485, HEIGHT * 0.86 + 30))
    screen.blit(menu_text, (400, 40))
    exit_btn = pygame.draw.rect(
        screen, gray, [WIDTH - 200, HEIGHT - 98, 180, 85], 0, 5)
    exit_text = label_font.render("Close", True, white)
    screen.blit(exit_text, (WIDTH - 160, HEIGHT - 70))
